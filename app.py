...
PASSWORD_HASH = "28f31acb726f186ebe2fb38ce388bc468f22b6815e35d7a3de4c659d239df921"
...
def check_password():
    if not st.session_state.authenticated:
        ...
        with col2:
            ...
            st.markdown("---")
            st.caption("Dashboard de Vendas Mensais | Jorge Vagarinho")
        return False
    return True
...
def _load_env_file():
    env_path = os.path.join(
        os.path.dirname(os.path.abspath(__file__)), "..", "atlas-credentials.env"
    )
    ...
    for sheet_name in xl.sheet_names:
        ...
        for row_idx in range(2, df.shape[0]):
            ...
            if not vendedor or vendedor == "nan":
                continue
            if (
                "acumulado" in vendedor.lower()
                or "total" in vendedor.lower()
                or vendedor == "0"
            ):
                continue
            ...
            for col_idx, day_num in day_col_mapping.items():
                ...
                val_sales = (
                    df.iloc[row_idx, pandas_col] if pandas_col < df.shape[1] else None
                )
                try:
                ...
                if client_col < df.shape[1]:
                    ...
                    try:
                        clients = (
                            int(float(val_clients)) if pd.notna(val_clients) else 0
                        )
                    except (ValueError, TypeError):
                        clients = 0
                else:
        ...
        for _, row in df.iterrows():
            ...
            if val0 and val0 != "nan" and val0.strip() != "" and val2 is not None:
                try:
                    ...
                    resp_display = (
                        current_resp.replace("Resp. Cobranca: ", "")
                        if current_resp
                        else "N/A"
                    )
                    resp_display = (
                        resp_display.replace("Resp. Cobran\u00e7a: ", "")
                        if current_resp
                        else "N/A"
                    )
                    results.append(
    ...
    for resp_text, data in daily_summary.items():
        ...
        try:
            ...
            updated_vendors.append(
                f"{vendor_code}: {data['total_vendas']:.2f} EUR ({data['clientes']} clientes)"
            )
        except Exception as e:
...
if db is None:
    ...
    if mongo_uri:
        ...
        st.sidebar.info(
            "Verifique se o IP do Render está na whitelist do MongoDB Atlas (Network Access -> 0.0.0.0/0)"
        )
    else:
        st.sidebar.warning(
            "MongoDB não configurado. Configure MONGODB_URI nas variáveis de ambiente."
        )

    uploaded_excel = st.sidebar.file_uploader(
        "Upload ficheiro Excel mensal para importar", type=["xlsx"]
    )
    if uploaded_excel:
        filepath = os.path.join(
            os.path.dirname(os.path.abspath(__file__)), "data", uploaded_excel.name
        )
...
if monthly_data:
    ...
    if month_data:
        ...
        vendedores_disponiveis = [
            v
            for v in month_data["vendedores_order"]
            if any(
                month_data["vendedores"][v]["daily_sales"].get(d, 0) != 0
                for d in filtered_days
            )
        ]

        selected_vendedores = st.sidebar.multiselect(
            "Vendedores", vendedores_disponiveis, default=vendedores_disponiveis
        )
else:
    st.sidebar.warning("Nenhum dado encontrado na base de dados.")
    st.sidebar.info("Faça upload do ficheiro Excel mensal para importar os dados.")

    uploaded_excel = st.sidebar.file_uploader(
        "Upload ficheiro Excel mensal", type=["xlsx"]
    )
    if uploaded_excel:
        filepath = os.path.join(
            os.path.dirname(os.path.abspath(__file__)), "data", uploaded_excel.name
...
with tab1:
    ...
    if month_data and filtered_days:
        ...
        for v in selected_vendedores:
            ...
            vendas_v = sum(
                v_data.get("daily_sales", {}).get(d, 0) for d in filtered_days
            )
            clientes_v = sum(
                v_data.get("daily_clients", {}).get(d, 0) for d in filtered_days
            )
            dias_trabalho = sum(
                1 for d in filtered_days if v_data.get("daily_sales", {}).get(d, 0) != 0
            )
        ...
        vendedores_ativos = sum(
            1 for v in selected_vendedores if vendedor_stats[v]["dias_trabalho"] > 0
        )
        ...
        with col_chart2:
            ...
            if table_data:
                ...
                totals = {
                    col: df_clients[col].sum()
                    for col in df_clients.columns
                    if col not in ["Vendedor"]
                }
                totals["Vendedor"] = "TOTAL"
                df_clients = pd.concat(
                    [df_clients, pd.DataFrame([totals])], ignore_index=True
                )
        ...
        with col_chart3:
            ...
            acum_data = {
                v: vendedor_stats[v]["vendas"]
                for v in selected_vendedores
                if vendedor_stats[v]["vendas"] > 0
            }
            if acum_data:
                fig_acum = go.Figure(
                    data=[
                        go.Pie(
                            ...
                            hole=0.3,
                            marker=dict(
                                colors=[get_vendedor_cor(v) for v in acum_data.keys()]
                            ),
                        )
                    ]
                )
        ...
        with col_chart4:
            ...
            media_data = {
                v.strip(): vendedor_stats[v]["media_diaria"]
                for v in selected_vendedores
                if vendedor_stats[v]["dias_trabalho"] > 0
            }
            if media_data:
                sorted_media = dict(
                    sorted(media_data.items(), key=lambda x: x[1], reverse=True)
                )
                fig_media = go.Figure(
                    data=[
                        go.Bar(
                            ...
                            orientation="h",
                            marker_color=[
                                get_vendedor_cor(v) for v in sorted_media.keys()
                            ],
                        )
                    ]
                )
        ...
        if weeks:
            week_names_sorted = sorted(
                weeks.keys(), key=lambda x: int(x.split()[1].split("/")[0])
            )
            ...
            for v in selected_vendedores:
                ...
                for wk_label, wk_days in weeks.items():
                    week_chart_data[v][wk_label] = sum(
                        v_data.get("daily_sales", {}).get(d, 0) for d in wk_days
                    )
            ...
            for wk in week_names_sorted:
                total_week_row[wk] = sum(
                    week_chart_data[v].get(wk, 0) for v in selected_vendedores
                )
        ...
        with st.expander("Comparação Mês a Mês"):
            ...
            if months_to_compare:
                ...
                fig_comp = make_subplots(
                    rows=1, cols=2, subplot_titles=("Vendas Totais", "Total Clientes")
                )
                fig_comp.add_trace(
                    go.Bar(
                        x=df_comp["Mês"],
                        y=df_comp["Total Vendas"],
                        marker_color="#1f77b4",
                        name="Vendas",
                    ),
                    row=1,
...
with tab2:
    ...
    uploaded_daily = st.file_uploader(
        "Carregar ficheiro Excel do dia (ex: 16-06-2026.xlsx)",
        type=["xlsx"],
        key="daily_upload",
    )
    ...
    if uploaded_daily:
        ...
        if df_daily is not None:
            ...
            resumo_resp = (
                df_daily.groupby("Responsável")
                .agg(Total=("Total Liq", "sum"), Clientes=("Total Liq", "count"))
                .reset_index()
            ...
            if daily_summary:
                ...
                for resp, data in daily_summary.items():
                    ...
                    st.write(
                        f"- **{label}**: {data['total_vendas']:,.2f} EUR ({data['clientes']} clientes)"
                    )

                reprocess = st.checkbox(
                    "Reprocessar mesmo que o dia já tenha sido processado"
                )

                if st.button(
                    "Atualizar Dados Mensais", type="primary", use_container_width=True
                ):
                    if reprocess:
                        db = get_db()
                        if db is not None:
                            ...
                            db.vendas.delete_many(
                                {"month": month_name_rp, "day": day_date.day}
                            )
                            db.processed_days.delete_many(
                                {"month": month_name_rp, "day": day_date.day}
                            )
